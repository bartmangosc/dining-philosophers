...
// Dining philosophers problem (using threads) - no starvation and no deadlock
...
sem_t waiter; // A semaphore simulating a waiter who can only allow 5 philosophers to "look" at the table at the same time
FILE *log_file;
...
void *philosopher(void *arg) {
	...
	f.fork_id[nr] = 0;
        ...
        while (!(f.fork_id[(nr + 1) % NUM_FORKS])) {
            pthread_cond_wait(&f.ok[nr], &f.mut);
            philosophers_states[nr] = HUNGRY;
        }
...
void *display_states(void *arg) {
    fprintf(log_file, "\nState of the philosophers (using threads) - no starvation and no deadlock:\n");
    fflush(log_file);
    while (1) {
        printf("\nState of the philosophers (using threads) - no starvation and no deadlock:\n");
...
int main() {
    sem_init(&waiter, 0, NUM_PHIL - 1);
    
    log_file = fopen("dpt_nostarvation_nodeadlock_log.txt", "w");
    if (!log_file) {
