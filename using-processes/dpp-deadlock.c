...
// Dining philosophers problem (using processes) - possible deadlock
...
void display_states() {
    fprintf(log_file, "State of the philosphers (using processes) - possible deadlock:\n");
    fflush(log_file);
    while (1) {
        printf("\nState of the philosphers (using processes) - possible deadlock:\n");
...
int main() {
    ...
    log_file = fopen("dpp_deadlock_log.txt", "w");
    if (!log_file) {
