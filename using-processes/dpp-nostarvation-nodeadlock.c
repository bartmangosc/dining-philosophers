...
// Dining philosophers (using processes) - no starvation and no deadlock

// States of the philosophers: thinking, hungry, eating, holding only left fork
typedef enum { THINKING, HUNGRY, EATING, LEFT} State;
...
int waiter_sem; // A semaphore simulating a waiter who can only allow 5 philosophers to "look" at the table at the same time
...
void philosopher(int nr) {
    while (1) {
        ...
        sem_op(waiter_sem, 0, -1);
        
        sem_op(table_sem, 0, -1);
	while (semctl(forks_sem[nr], 0, GETVAL) == 0){
	     shared->philosophers_states[nr] = HUNGRY;
	}
        sem_op(forks_sem[nr], 0, -1);
        shared->philosophers_states[nr] = LEFT;
...
void display_states() {
    fprintf(log_file, "\nState of the philosophers (using processes) - no starvation and no deadlock:\n");
    fflush(log_file);
    while (1) {
        printf("\nState of the philosophers (using processes) - no starvation and no deadlock:\n");
...
int main() {
    ...
    waiter_sem = semget(IPC_PRIVATE, 1, IPC_CREAT | 0666);
    semctl(waiter_sem, 0, SETVAL, NUM_PHIL - 1);
    ...
    log_file = fopen("dpp_nostarvation_nodeadlock_log.txt", "w");
    if (!log_file) {
